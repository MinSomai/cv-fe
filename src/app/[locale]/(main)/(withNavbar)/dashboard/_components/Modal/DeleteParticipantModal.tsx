...
const DeleteParticipantModal: React.FC<{
  isModalOpen: boolean;
  ...
  const handleCheckboxChange = () => {
    ...
    if (user?.id) {
      ...
      localData.deleteModalOpen = false;
  ...
  const handleDeleteClick = async () => {
    try {
      await rest(
        `${process.env.NEXT_PUBLIC_PAYLOAD_URL}/api/users/${deleteParticipant}`,
        {},
        {
          method: "DELETE",
        }
      ...
      <DialogContent className="max-w-[544px] inter">
        <DialogHeader>
          ...
          <DialogDescription className="text-[#475467]">
            Are you sure you want to delete this participant? All associated
