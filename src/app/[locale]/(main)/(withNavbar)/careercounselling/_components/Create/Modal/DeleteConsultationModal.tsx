...
const DeleteConsultationModal: React.FC<{
  isModalOpen: boolean;
  ...
  const handleCheckboxChange = () => {
    ...
    if (user?.id) {
      ...
      localData.deleteConsultationModalOpen = false;
  ...
  const handleDeleteClick = async () => {
    try {
      await rest(
        `${process.env.NEXT_PUBLIC_PAYLOAD_URL}/api/counsellingInterviews/${deleteConsultation}`,
        {},
        {
          method: "DELETE",
        }
      ...
      <DialogContent className="max-w-[544px] inter">
        <DialogHeader>
          ...
          <DialogDescription className="text-[#475467]">
            Are you sure you want to delete this career path consultation? All
            associated information will also be permanently deleted. This action
            cannot be undone.
          </DialogDescription>
        </DialogHeader>
