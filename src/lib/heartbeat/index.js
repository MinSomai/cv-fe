...
export class Heartbeat {
  ...
  async init() {
    try {
      this.webcamVideoElement.width = 640;
      this.webcamVideoElement.height = 480;
      this.frameRGB = new cv.Mat(
        this.webcamVideoElement.height,
        this.webcamVideoElement.width,
        cv.CV_8UC4
      );
      this.lastFrameGray = new cv.Mat(
        this.webcamVideoElement.height,
        this.webcamVideoElement.width,
        cv.CV_8UC1
      );
      ...
      this.rppgTimer = setInterval(() => {
        if (this.faceValid) {
          const result = this.rppg();
          // Do something with the result, e.g., store it in a variable
          // console.log("rppg returned:", result);
          // Optionally, assign it to a class property:
          this.lastRppgResult.push(result);
        } else {
          this.lastRppgResult.push(undefined);
        }
      }, this.rppgInterval);
    } catch (e) {
      console.log(e);
  ...
  estimateAffineTransform(corners_0, corners_1) {
    ...
    let t_x = cv.matFromArray(
      corners_0.rows * 2,
      1,
      cv.CV_32FC1,
      Array.from(corners_0.data32F)
    );
    let y = cv.matFromArray(
      corners_1.rows * 2,
      1,
      cv.CV_32FC1,
    ...
    let t_01 = new cv.Mat();
    cv.repeat(
      cv.matFromArray(2, 1, cv.CV_32FC1, [1, 0]),
      corners_0.rows,
      1,
      t_10
    );
    cv.repeat(
      cv.matFromArray(2, 1, cv.CV_32FC1, [0, 1]),
      corners_0.rows,
      1,
      t_01
    );
  ...
  standardize(signal) {
    ...
    let means_c3 = cv.matFromArray(1, 1, cv.CV_32FC3, [
      mean.data64F[0],
      mean.data64F[1],
      mean.data64F[2],
    ]);
    let stdDev_c3 = cv.matFromArray(1, 1, cv.CV_32FC3, [
      stdDev.data64F[0],
      stdDev.data64F[1],
      stdDev.data64F[2],
    ]);
