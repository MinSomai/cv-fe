...
// Map locales to country codes for flag icons
const localeToCountryCode: Partial<Record<Locale, string>> = {
  en: "gb", // UK flag
  ru: "ru",
  he: "il",
};
...
export const LocaleSwitcher = () => {
  ...
  const switchLocale = (newLocale: Locale) => {
    ...
    // To change locale, we need to navigate to the same path with the new locale
    const currentPath = pathname || "/";
    // Construct the full path with the new locale
    // Use window.location to trigger a full navigation which will reload with new locale
  ...
  const currentCountryCode = localeToCountryCode[locale] || locale || "gb";
          ...
          <div>
            ...
            >
              <div className="flex flex-row items-center">
                <FlagIcon countryCode={currentCountryCode} />
                {localeNames[locale]}
              </div>
              <svg
            ...
            >
              <div className="py-1 grid grid-cols-1 gap-2" role="none">
                {locales.map((loc, index) => {
                  const countryCode = localeToCountryCode[loc] || loc || "gb";
                  return (
                    <button
                      key={loc}
                      ...
                      className={cn(
                        ...
                        {
                          "bg-gray-100 text-gray-900": locale === loc,
                          "text-gray-700": locale !== loc,
                        },
                        { "rounded-r": index % 2 === 0 },
                        { "rounded-l": index % 2 !== 0 }
                      )}
